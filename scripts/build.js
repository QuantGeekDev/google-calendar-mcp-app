#!/usr/bin/env node

import * as esbuild from 'esbuild';
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';

const __dirname = dirname(fileURLToPath(import.meta.url));
const isWatch = process.argv.includes('--watch');

/** @type {import('esbuild').BuildOptions} */
const buildOptions = {
  entryPoints: [join(__dirname, '../src/index.ts')],
  bundle: true,
  platform: 'node',
  target: 'node18',
  outfile: join(__dirname, '../build/index.js'),
  format: 'esm',
  banner: {
    js: '#!/usr/bin/env node\n',
  },
  packages: 'external', // Don't bundle node_modules
  sourcemap: true,
};

/** @type {import('esbuild').BuildOptions} */
const authServerBuildOptions = {
  entryPoints: [join(__dirname, '../src/auth-server.ts')],
  bundle: true,
  platform: 'node',
  target: 'node18',
  outfile: join(__dirname, '../build/auth-server.js'),
  format: 'esm',
  packages: 'external', // Don't bundle node_modules
  sourcemap: true,
};

if (isWatch) {
  const context = await esbuild.context(buildOptions);
  const authContext = await esbuild.context(authServerBuildOptions);
  await Promise.all([context.watch(), authContext.watch()]);
  process.stderr.write('Watching for changes...\n');
} else {
  await Promise.all([
    esbuild.build(buildOptions),
    esbuild.build(authServerBuildOptions)
  ]);

  // Make the file executable on non-Windows platforms
  if (process.platform !== 'win32') {
    const { chmod } = await import('fs/promises');
    await chmod(buildOptions.outfile, 0o755);
  }

  // Copy static files (HTML, CSS, etc.) to build directory
  const { mkdir, copyFile } = await import('fs/promises');
  const webDir = join(__dirname, '../build/web');
  await mkdir(webDir, { recursive: true });

  const staticFiles = [
    'accounts.html',
    'auth-landing.html',
    'auth-success.html',
    'auth-error.html',
    'styles.css'
  ];

  await Promise.all(staticFiles.map(file =>
    copyFile(
      join(__dirname, '../src/web', file),
      join(webDir, file)
    )
  ));
  process.stderr.write('Static files copied to build directory\n');

  // Copy MCP Apps dist to build directory (if built)
  // Each app is in dist/apps/<app-name>/index.html
  const { existsSync, statSync } = await import('fs');
  const appsDistDir = join(__dirname, '../dist/apps');
  const buildAppsDir = join(__dirname, '../build/apps');
  if (existsSync(appsDistDir)) {
    const { readdir } = await import('fs/promises');
    const appDirs = await readdir(appsDistDir);
    let copiedCount = 0;
    for (const appDir of appDirs) {
      const appPath = join(appsDistDir, appDir);
      const indexPath = join(appPath, 'index.html');
      if (statSync(appPath).isDirectory() && existsSync(indexPath)) {
        const destDir = join(buildAppsDir, appDir);
        await mkdir(destDir, { recursive: true });
        await copyFile(indexPath, join(destDir, 'index.html'));
        copiedCount++;
      }
    }
    process.stderr.write(`MCP Apps: ${copiedCount} UI apps copied to build directory\n`);
  }
} 