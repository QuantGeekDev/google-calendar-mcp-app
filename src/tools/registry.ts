...
import { getToolUIMeta } from "../apps/ui-resources.js";
...
export class ToolRegistry {
  ...
  private static registerSingleTool(
    ...
    // Build tool metadata, including MCP Apps UI reference if available
    const toolMeta: Record<string, any> = {
      description: tool.description,
      inputSchema: this.extractSchemaShape(tool.schema)
    };

    // Add _meta.ui for MCP Apps support
    const uiMeta = getToolUIMeta(tool.name);
    if (uiMeta) {
      toolMeta._meta = uiMeta;
    }
