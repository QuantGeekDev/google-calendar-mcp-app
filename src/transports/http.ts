...
export class HttpTransportHandler {
  ...
  async connect(): Promise<void> {
    ...
    const httpServer = http.createServer(async (req, res) => {
      ...
      // Origin validation: allow localhost and tunneled requests (no Origin header)
      // Tunnels like cloudflared/ngrok strip or don't send Origin headers
      if (origin && !isLocalhostOrigin(origin) && !origin.includes('trycloudflare.com') && !origin.includes('ngrok')) {
      ...
      // Handle CORS - allow origin for localhost, tunnels, and Claude.ai
      const allowedCorsOrigin = origin || '*';
      res.setHeader('Access-Control-Allow-Origin', allowedCorsOrigin);
      res.setHeader('Access-Control-Allow-Methods', 'GET, POST, DELETE, OPTIONS');
      res.setHeader('Access-Control-Allow-Headers', 'Content-Type, mcp-session-id, mcp-protocol-version');
      res.setHeader('Access-Control-Expose-Headers', 'Content-Type, mcp-session-id, mcp-protocol-version');
      ...
      // Stateless per-request transport pattern
      // Close previous transport before connecting new one (SDK 1.27+ requirement)
      ...
      try {
        ...
        // Ignore close errors (may not be connected yet)
      }
